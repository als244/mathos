...
int main(int argc, char * argv[]){
	...
	// 1.) PD based on inputted configuration
	 ...
	 RDMAConnectionType connection_type = RDMA_RC;
        ...
        qp_attr.cap.max_send_wr = 1U << 3;  // increase if you want to keep more send work requests in the SQ.
        qp_attr.cap.max_recv_wr = 1U << 3;  // increase if you want to keep more receive work requests in the RQ.
	...
	ret = setup_connection(RDMA_RC, 0, SERVER_ID, SERVER_IP, SERVER_PORT, NULL, NULL, NULL, CLIENT_ID, CLIENT_IP, pd, qp, cq, &conn);
	if (ret != 0){
