...
// Allocating all the memory here to make it easy during processing of tcp_rdma_init (process_rdma_init_connection)
Net_Node * init_remote_node_container(Net_World * net_world, uint32_t node_id, uint32_t num_ports, uint32_t num_endpoints) {
	...
	node -> num_endpoints = num_endpoints;


	// 2.) allocate remote ports container

	Net_Port * remote_ports = (Net_Port *) malloc(num_ports * sizeof(Net_Port));
	...
	for (uint32_t i = 0; i < num_ports; i++){
		...
		if (remote_ports[i].address_handles == NULL){
			...
			for (uint32_t k = 0; k < i - 1; k++){
				free(remote_ports[k].address_handles);
			}
			free(remote_ports);
			free(node);
			return NULL;
	...
	Net_Endpoint * remote_endpoints = (Net_Endpoint *) malloc(num_endpoints * sizeof(Net_Endpoint));
	if (remote_endpoints == NULL){
		fprintf(stderr, "Error: malloc failed allocating endpoints array for remote node\n");
		for (uint32_t k = 0; k < num_ports; k++){
			free(remote_ports[k].address_handles);
		}
		free(remote_ports);
		free(node);
