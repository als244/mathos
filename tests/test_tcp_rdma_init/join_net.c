...
int process_join_net(Self_Net * self_net, int sockfd, bool * ret_is_join_successful, Join_Response ** ret_join_response, Net_World ** ret_net_world) {
...
int join_net(Self_Net * self_net, char * master_ip_addr, Join_Response ** ret_join_response, Net_World ** ret_net_world) {
	...
	while (!is_join_successful){
		...
		ret = process_join_net(self_net, client_sockfd, &is_join_successful, ret_join_response, ret_net_world);
		if (ret == -1){
