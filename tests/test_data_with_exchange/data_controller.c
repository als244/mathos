...
Data_Controller * init_data_controller(uint32_t self_id, Inventory * inventory, uint32_t max_connections, int num_cqs, struct ibv_context * ibv_ctx){
	...
	qp_attr.cap.max_send_wr = 1U << 4;  // increase if you want to keep more send work requests in the SQ.
	qp_attr.cap.max_recv_wr = 1U << 4;  // increase if you want to keep more receive work requests in the RQ.
